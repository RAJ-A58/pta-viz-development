...
class QLFCPAWidget(QWidget):
    ...
    def setAnalysisMode(self, mode: str):
        self.analysisMode = mode
        if mode == 'vasco':
            self.PTAFp = './results/vasco/pta/iter_'
            self.switchAnalysisButton.hide() # VASCO is PTA only, hide the Liveness toggle
            self.PTAAnalysis = True
            if self.dataSplitter.widget(0) != self.PTAResults:
                self.dataSplitter.replaceWidget(0, self.PTAResults)
        else:
            self.PTAFp = './results/lfcpa/pta/iter_'
            self.LivenessFp = './results/lfcpa/la/iter_'
            self.switchAnalysisButton.show()

    def getPath(self, iter:int, round:int, stmt:int):
            if self.analysisMode == 'vasco':
                # VASCO doesn't use LA/PTA "rounds", so we remove it from the filename.
                # We use iter-1 because standard PTA engines (like FSPTA) save starting at iter_0
                return str(iter - 1) + 'stmt_' + str(stmt)
            else:
