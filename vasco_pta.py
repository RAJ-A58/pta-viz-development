...
def vasco_pta_proc(struct_dict, func_dict, context, result_dest, startedAnalysis:dict[str, list], completedAnalysis:dict[str, list]):
    ...
    while change:
        ...
        for stmt in stmt_lst:
            ...
            # BUG 1 FIXED: changed is_is_stmt_type to is_stmt_type
                ...
                if not change:
                    ...
                    for ca_context in completedAnalysis[funcID]:
                        if is_equal(ca_context[0], ptr_dict_out):
                            ...
                            found = True
                    if not found:
                        # BUG 2 FIXED: Replaced 'stmt.' with a list comprehension of the actual argument names
                        actual_args = [arg.varName for arg in stmt.args]
                        ptr_dicts[stmt_no] = copy.deepcopy(vasco_pta_proc(struct_dict, func_dict, (funcID, ptr_dict_out, actual_args), result_dest, startedAnalysis, completedAnalysis))
                stmt_no += 1
                continue
